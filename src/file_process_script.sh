...
main() {
    ...
    local BUCKET_NAME="fovus-drake-store"

    # Retrieve data from DynamoDB using AWS CLI
    input_file_path=$(aws dynamodb get-item --table-name upload_files --key '{"id": {"S": "'"$id"'"}}' --query 'Item.input_file_path.S' --region 'us-east-2')
    input_text=$(aws dynamodb get-item --table-name upload_files --key '{"id": {"S": "'"$id"'"}}' --query 'Item.input_text.S' --region 'us-east-2')
    ...
    aws dynamodb put-item --table-name upload_files --item '{"id": {"S": "'"${id}-1"'"}, "output_file_path": {"S": "'"${BUCKET_NAME}/${output_file_name}"'"}}' --region 'us-east-2'
